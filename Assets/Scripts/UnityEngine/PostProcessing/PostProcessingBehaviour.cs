...
namespace UnityEngine.PostProcessing
{
	...
	public class PostProcessingBehaviour : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			{
				flag = true;
				DepthOfFieldComponent depthOfField = m_DepthOfField;
				RenderTexture source2 = renderTexture;
				Material uberMaterial = material;
				bool antialiasCoC = flag2;
				Vector2 jitterVector = m_Taa.jitterVector;
				AntialiasingModel.Settings settings = m_Taa.model.settings;
				depthOfField.Prepare(source2, uberMaterial, antialiasCoC, jitterVector, settings.taaSettings.motionBlending);
			}
			if (m_Bloom.active)
			{
				flag = true;
		...
		private void RemoveCommandBuffer<T>() where T : PostProcessingModel
		{
			...
			if (m_CommandBuffers.TryGetValue(typeFromHandle, out KeyValuePair<CameraEvent, CommandBuffer> value))
			{
				m_Camera.RemoveCommandBuffer(value.Key, value.Value);
		...
		{
			if (!m_CommandBuffers.TryGetValue(typeof(T), out KeyValuePair<CameraEvent, CommandBuffer> value))
			{
